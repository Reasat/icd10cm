...
- `make dependencies` / `just dependencies` installs `linkml-owl==0.5.0` and git `linkml` / `linkml-runtime` from `main` for the linkml-runtime comma-in-synonym workaround; CI runs this after `uv sync`.
